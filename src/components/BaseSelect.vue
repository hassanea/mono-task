<template>
  <base-form-control>
    ...
    >
      <select
        class="read-only:bg-form-light read-only:dark:bg-form-dark read-only:dark:text-light read-only:text-dark font-sans2 dark:border-form-border-dark border-form-border-light focus:border-highlight active:bg-selection active:text-dark hover:dark:text-light hover:text-dark pointer-events-auto mx-0 mb-1 block h-20 w-full max-w-150 min-w-70 cursor-grab appearance-none rounded-lg border-2 border-solid p-2.5 text-center text-[0.94rem] leading-normal font-medium not-italic transition-all duration-100 ease-in-out select-auto hover:bg-[#8B87BD] hover:font-bold focus:border-4 focus:border-solid focus:outline-none active:font-bold disabled:pointer-events-none disabled:bg-gray-700 disabled:opacity-75 disabled:select-none md:p-3 hover:dark:bg-[#343159]"
        :class="selectClasses"
        v-model="model"
        :name="selectName"
