...
export const useMyColorModeStore = defineStore('useMyColorModeStore', () => {
  ...
  function setMyColorMode(fallback: string, storageOption: string) {
    ...
    if (
      ...
      (!hasColorModeInStorage && (isDarkMode.value || isLightMode.value))
    ) {
  ...
  const setColorSwitchIcon = computed(() => {
    return colorMode.value === 'dark' ? 'fa-solid fa-moon' : 'fa-solid fa-sun'
  })
  ...
  // Client-Side
  // cleanUpMyColorMode watcher function to removed from reactive effects when app.vue component is unmounted.
  const cleanUpMyColorMode = watch([colorMode, isDarkMode], (values) => {
